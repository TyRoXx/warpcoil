...
namespace
{
    struct websocket_client : boost::intrusive::list_base_hook<
    ...
    struct web_site_publisher : async_publisher
    {
        typedef boost::intrusive::list<
            websocket_client, boost::intrusive::constant_time_size<false>> websocket_client_list;
                                 ...
                                 boost::string_ref const content_type)
    {
        ...
        client->response.fields.insert(
            "Content-Length", boost::lexical_cast<std::string>(client->response.body.size()));
        client->response.fields.insert("Content-Type", content_type);
        beast::http::async_write(
            client->socket, client->response,
            [client, is_keep_alive, &server_impl, document_root](boost::system::error_code const ec)
            {
                Si::throw_if_error(ec);
                if (is_keep_alive)
                {
    ...
    {
        Si::visit<void>(
            ventura::read_file(ventura::safe_c_str(ventura::to_os_string(served_document))),
    ...
    {
        auto code_writer =
            Si::Sink<char, Si::success>::erase(Si::make_container_sink(client->response.body));
        ...
        warpcoil::types::interface_definition const service =
            warpcoil::create_publisher_interface();
        Si::append(code_writer, "var make_client = ");
        warpcoil::javascript::generate_client(code_writer, warpcoil::indentation_level(), service,
                                              library);
        Si::append(code_writer, ";\n");

        client->response.reason = "OK";
        client->response.status = 200;
        serve_prepared_response(client, is_keep_alive, server_impl, document_root,
                                "text/javascript");
    }
        ...
        beast::http::async_read(
            ...
            [client, &server_impl, document_root](boost::system::error_code const ec)
            {
                ...
                if (beast::http::is_upgrade(client->request))
                {
                    ...
                    new_client->websocket.async_accept(
                        client->request,
                ...
                {
                    auto new_client = std::make_shared<file_client>(
                        std::move(client->socket), std::move(client->receive_buffer));
                    if (client->request.url == "/index.js")
                    {
                        serve_static_file(new_client, beast::http::is_keep_alive(client->request),
                                          server_impl, document_root,
                                          document_root / ventura::relative_path("index.js"),
                                          "text/javascript");
                    }
                    else if (client->request.url == "/service.js")
                    {
                        serve_interface_library(new_client,
                    ...
                    else
                    {
                        serve_static_file(new_client, beast::http::is_keep_alive(client->request),
                                          server_impl, document_root,
                                          document_root / ventura::relative_path("index.html"),
                                          "text/html");
                    }
                }
            });
                      ...
                      ventura::absolute_path const &document_root)
    {
        auto client = std::make_shared<http_client>(
            boost::asio::ip::tcp::socket(acceptor.get_io_service()), beast::streambuf());
        acceptor.async_accept(client->socket, [&acceptor, client, &server_impl,
