...
BOOST_AUTO_TEST_CASE(async_client_pipelining_simple)
{
    ...
    acceptor.async_accept(
        accepted_socket,
        [&accepted_socket, &server_impl, &served_0, &served_1, &got_1](boost::system::error_code ec)
        {
            ...
            server->serve_one_request(
                [server, writer, splitter, &served_0, &served_1,
                 &got_1](boost::system::error_code ec)
                {
                    BOOST_REQUIRE_EQUAL(boost::system::error_code(), ec);
                    served_0.enter();
                    served_1.enable();
                    got_1.enable();
                    server->serve_one_request(
                        [server, writer, splitter, &served_1](boost::system::error_code ec)
                        {
                            BOOST_REQUIRE_EQUAL(boost::system::error_code(), ec);
                            served_1.enter();
                        });
    ...
    socket.async_connect(boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v4::loopback(),
...
namespace
{
    ...
    void
    serve_n(ServerPtr server,
            std::shared_ptr<warpcoil::cpp::message_splitter<boost::asio::ip::tcp::socket>>
                server_splitter,
    ...
    void count_up(Client &client, std::uint16_t const first, std::uint16_t const last)
    {
        ...
        client.variant(
            ...
            {
                BOOST_REQUIRE_EQUAL((Si::variant<std::uint16_t, std::string>{
...
BOOST_AUTO_TEST_CASE(async_client_pipelining_many_requests_in_sequence)
{
    ...
    acceptor.async_accept(
        accepted_socket,
        [&accepted_socket, &server_impl, request_count](boost::system::error_code ec)
        {
            BOOST_REQUIRE_EQUAL(boost::system::error_code(), ec);
            auto splitter =
                std::make_shared<warpcoil::cpp::message_splitter<boost::asio::ip::tcp::socket>>(
        ...
        [&client, &writer, request_count, &got_responses](boost::system::error_code ec)
        {
            ...
            for (std::size_t i = 1; i <= request_count; ++i)
            {
                ...
                client.variant(static_cast<std::uint32_t>(i),
                               [i, &got_responses](
                                   Si::error_or<Si::variant<std::uint16_t, std::string>> result)
                               {
                                   BOOST_REQUIRE_EQUAL((Si::variant<std::uint16_t, std::string>{
