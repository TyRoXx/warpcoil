...
namespace
{
    struct local_directory : async_directory
    {
        ...
        void remove(std::string path, std::function<void(boost::system::error_code, std::tuple<>)> on_result) override
        {
...
BOOST_AUTO_TEST_CASE(file_api_directory)
{
    ...
    got_result.enable();
    directory.make_directory("make_directory_test", [&got_result](boost::system::error_code ec, std::tuple<>)
                             {
                                 BOOST_CHECK(!ec);
                                 got_result.enter();
                             });
