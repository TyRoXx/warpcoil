...
namespace warpcoil
{
    struct impl_test_interface : async_test_interface
    {
        ...
        void
        real_multi_parameters(std::string first, std::uint16_t second,
        ...
        void atypical_int(std::uint16_t argument,
                          std::function<void(Si::error_or<std::uint16_t>)> on_result) override
        {
            on_result(argument);
        }

        void variant(Si::variant<std::uint32_t, std::string> argument,
                     std::function<void(Si::error_or<Si::variant<std::uint16_t, std::string>>)>
                         on_result) override
        {
            on_result(Si::visit<Si::variant<std::uint16_t, std::string>>(
                argument,
                [](std::uint32_t value)
                {
