...
BOOST_AUTO_TEST_CASE(bidirectional)
{
    auto const allocations_before = warpcoil::number_of_allocations();
    {
        ...
        warpcoil::checkpoint request_answered_a;
        warpcoil::checkpoint request_answered_b;
        auto splitter_a = std::make_shared<warpcoil::cpp::message_splitter<decltype(a_reading)>>(a_reading);
        auto writer_a = std::make_shared<warpcoil::cpp::buffered_writer<decltype(a_writing)>>(a_writing);
        auto client_a = std::make_shared<async_test_interface_client<decltype(a_writing), decltype(a_reading)>>(
            *writer_a, *splitter_a);
        ...
        BOOST_TEST_MESSAGE("Sent request on A");
        BOOST_CHECK_EQUAL(1u, pending_requests(*client_a));
        server_a->serve_one_request([server_a, splitter_a, writer_a, client_a](boost::system::error_code ec)
                                    {
                                        BOOST_TEST_MESSAGE("Served request A");
                                        BOOST_REQUIRE_EQUAL(boost::system::error_code(), ec);
                                    });
        ...
        async_test_interface_client<decltype(b_writing), decltype(b_reading)> client_b(writer_b, splitter_b);
        {
            BOOST_CHECK_EQUAL(0u, pending_requests(client_b));
        ...
        std::array<std::uint8_t, 16> const request_to_b = {{0, 0, 0, 0, 0, 0, 0, 0, 0, 4, 'u', 't', 'f', '8', 1, 'Y'}};
        BOOST_REQUIRE_EQUAL_COLLECTIONS(request_to_b.begin(), request_to_b.end(), a_writing.written[0].begin(),
                                        a_writing.written[0].end());
        a_writing.written.clear();
        BOOST_REQUIRE_EQUAL(1u, b_writing.written.size());
        std::array<std::uint8_t, 16> const request_to_a = {{0, 0, 0, 0, 0, 0, 0, 0, 0, 4, 'u', 't', 'f', '8', 1, 'X'}};
        BOOST_REQUIRE_EQUAL_COLLECTIONS(request_to_a.begin(), request_to_a.end(), b_writing.written[0].begin(),
                                        b_writing.written[0].end());
        ...
        BOOST_REQUIRE(b_reading.respond);
        std::array<std::uint8_t, 14> const response_to_a = {{1, 0, 0, 0, 0, 0, 0, 0, 0, 4, 'Y', '1', '2', '3'}};
        std::array<std::uint8_t, 14> const response_to_b = {{1, 0, 0, 0, 0, 0, 0, 0, 0, 4, 'X', '1', '2', '3'}};
        BOOST_REQUIRE_EQUAL(1u, a_writing.written.size());
        BOOST_REQUIRE_EQUAL_COLLECTIONS(response_to_b.begin(), response_to_b.end(), a_writing.written[0].begin(),
                                        a_writing.written[0].end());
        BOOST_REQUIRE_EQUAL(1u, b_writing.written.size());
        BOOST_REQUIRE_EQUAL_COLLECTIONS(response_to_a.begin(), response_to_a.end(), b_writing.written[0].begin(),
                                        b_writing.written[0].end());
        Si::exchange(a_writing.handle_result, nullptr)(boost::system::error_code(), response_to_b.size());
        ...
        Si::exchange(b_writing.handle_result, nullptr)(boost::system::error_code(), response_to_a.size());
    ...
    auto const allocations_after = warpcoil::number_of_allocations();
    BOOST_CHECK_LE((allocations_after - allocations_before), 200u);
}
...
BOOST_AUTO_TEST_CASE(bidirectional_request_merged_with_response_regression_test)
{
    auto const allocations_before = warpcoil::number_of_allocations();
    {
        ...
        std::array<std::uint8_t, 14> const response_to_b = {{1, 0, 0, 0, 0, 0, 0, 0, 0, 4, 'X', '1', '2', '3'}};
        std::copy(response_to_b.begin(), response_to_b.end(), everything_to_b.begin() + request_to_b.size());
        BOOST_REQUIRE_EQUAL(1u, pending_requests(client_b));
