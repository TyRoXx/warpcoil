...
namespace warpcoil
{
    inline bool update_generated_file(char const *file, Si::memory_range new_content,
                                      std::ostream &log)
    {
    ...
    inline int
    run_code_generator_command_line_tool(Si::iterator_range<char **> command_line_arguments,
                                         std::ostream &log, Si::memory_range code)
    {
        ...
        Si::optional<ventura::absolute_path> const clang_format =
            (command_line_arguments.size() >= 3)
                ? ventura::absolute_path::create(command_line_arguments[2])
                : Si::none;
        Si::optional<ventura::absolute_path> const clang_format_config_dir =
            (command_line_arguments.size() >= 4)
                ? ventura::absolute_path::create(command_line_arguments[3])
                : Si::none;
        ...
        if (clang_format)
        {
            ...
            {
                log << file << ": Command line argument for the clang-format "
                               "config directory missing\n";
                return 1;
            }
            log << file << " Formatting the code (originally " << code.size() << " bytes)\n";
            auto formatted_code_writer =
                Si::Sink<char, Si::success>::erase(Si::make_container_sink(formatted_code));
