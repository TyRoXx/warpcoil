...
namespace warpcoil
{
    namespace javascript
    {
        ...
        void generate_integer_parser(CharSink &&code, indentation_level indentation,
                                     std::size_t const number_of_bytes)
        {
            bool const return_64_bit = (number_of_bytes > 4);
            if (return_64_bit)
            {
                Si::append(code, "function (minimum_high, minimum_low, "
                                 "maximum_high, maximum_low)\n");
            }
            else
            {
                          ...
                          [&](indentation_level const in_result)
                          {
                              ...
                              block(code, in_result,
                                    [&](indentation_level const in_if)
                                    {
                                        ...
                                        {
                                            // TODO range check
                                            start_line(code, in_if, "return {high: result_high, "
                                                                    "low: result_low};\n");
                                        }
                                        else
                                        {
                                            start_line(code, in_if, "if ((result < minimum) || "
                                                                    "(result > maximum))\n");
                                            block(code, in_if,
                                                  [&](indentation_level const in_check)
                                                  {
                                                      start_line(code, in_check, "throw new "
                                                                                 "Error(\"received "
                                                                                 "integer out of "
                                                                                 "range\");\n");
                                                  },
            ...
            block(code, indentation,
                  [&](indentation_level const in_state)
                  {
                      ...
                      block(code, in_state,
                            [&](indentation_level const in_if)
                            {
                                start_line(code, in_if, "throw new "
                                                        "Error(\"Unexpected "
                                                        "response: \" + key);\n");
                            },
                            "\n");
                      start_line(code, in_state, "delete pending_requests[key];\n");
                      start_line(code, in_state, "var argument_parser = "
                                                 "pending_request.argument_"
                                                 "parser;\n");
                      ...
                      block(code, in_state,
                            [&](indentation_level const in_if)
                            {
                                ...
                                start_line(code, in_if,
                                           "pending_request.on_result(undefined, []);\n");
                                start_line(code, in_if, "return;\n");
                            },
                            "\n");
                      ...
                      block(code, in_state,
                            [&](indentation_level const in_state)
                            {
                                start_line(code, in_state,
                                           "var parsed = argument_parser(input);\n");
        ...
        template <class CharSink>
        void generate_parser_creation(CharSink &&code, indentation_level const indentation,
                                      types::type const &parsed, Si::memory_range const library)
        {
            Si::visit<void>(
                parsed,
                [&](types::integer const range)
                {
                    ...
                    case 8:
                        ...
                        Si::append(code, ", ");
                        Si::append(code,
                                   boost::lexical_cast<std::string>(range.minimum & 0xffffffffu));
                        Si::append(code, ", ");
                        Si::append(code, boost::lexical_cast<std::string>(range.maximum >> 32u));
                        Si::append(code, ", ");
                        Si::append(code,
                                   boost::lexical_cast<std::string>(range.maximum & 0xffffffffu));
                        break;
                    ...
                    else
                    {
                        ...
                        block(
                            ...
                            [&](indentation_level const in_function)
                            {
                                ...
                                generate_parser_creation(code, in_function, root->elements[0],
                                                         library);
                                Si::append(code, ";\n");
                                start_line(code, in_function, "return function (input)\n");
                                block(
                                                ...
                                                for (size_t i = 0; i < root->elements.size(); ++i)
                                                {
                                                    ...
                                                    block(
                                                        ...
                                                        [&](indentation_level const in_case)
                                                        {
                                                            ...
                                                            {
                                                                start_line(code, in_case, "current_"
                                                                                          "parser "
                                                                                          "= ");
                                                                generate_parser_creation(
                                                                    code, in_case,
                                                                    root->elements[i + 1], library);
                                                                Si::append(code, ";\n");
                                                                start_line(code, in_case,
                                                                           "break;\n");
                                                            }
                              ...
                              block(code, in_function,
                                    [&](indentation_level const in_function_2)
                                    {
                                        ...
                                        block(code, in_function_2,
                                              [&](indentation_level const in_if)
                                              {
                                                  start_line(code, in_if, "var status = "
                                                                          "length(input);\n");
                                                  start_line(code, in_if, "if (status === "
                                                                          "undefined) { return; "
                                                                          "}\n");
                                                  start_line(code, in_if, "content = new "
                                                                          "ArrayBuffer(status);"
                                                                          "\n");
                                              },
                                              "\n");
                                        ...
                                        block(code, in_function_2,
                                              [&](indentation_level const in_else)
                                              {
                                                  start_line(code, in_else, "new "
                                                                            "Uint8Array(content)["
                                                                            "received_content] = "
                                                                            "input;\n");
                                                  start_line(code, in_else,
                                                             "++received_content;\n");
                                                  start_line(code, in_else, "if (received_content "
                                                                            "!== "
                                                                            "content.byteLength) { "
                                                                            "return; }\n");
                                                  ...
                                                  start_line(code, in_else,
                                                             "for (var i = 0; i < "
                                                             "content.byteLength; ++i) "
                                                             "{ result += "
                                                             "String.fromCharCode(new "
                    ...
                    else
                    {
                        ...
                        block(
                            ...
                            [&](indentation_level const in_function)
                            {
                                ...
                                start_line(code, in_function, "var current_parser = ");
                                generate_parser_creation(code, in_function, root->elements[0].what,
                                                         library);
                                Si::append(code, ";\n");
                                start_line(code, in_function, "return function (input)\n");
                                block(
                                                    ...
                                                    block(
                                                        ...
                                                        [&](indentation_level const in_case)
                                                        {
                                                            ...
                                                            start_line(code, in_case, "result.",
                                                                       root->elements[i].name,
                                                                       " = status;\n");
                                                            start_line(code, in_case, "++members_"
                                                                                      "parsed;\n");
                                                            if (i == (root->elements.size() - 1))
                                                            {
                                                                start_line(code, in_case, "return "
                                                                                          "result;"
                                                                                          "\n");
                                                            }
                                                            else
        ...
        template <class CharSink>
        void generate_size_of_value(CharSink &&code, indentation_level indentation,
                                    types::type const &type, Si::memory_range const value,
            ...
            Si::visit<void>(
                ...
                [&](types::integer const range)
                {
                    Si::append(code, boost::lexical_cast<std::string>(
                                         find_number_of_bytes_required(range.maximum)));
                },
                [&](std::unique_ptr<types::variant> const &)
                {
                    throw std::logic_error("todo");
                ...
                [&](std::unique_ptr<types::vector> const &root)
                {
                    Si::append(code, "(");
                    Si::append(code, boost::lexical_cast<std::string>(
                                         find_number_of_bytes_required(root->length.maximum)));
                    Si::append(code, " + ");
                    Si::append(code, value);
                    Si::append(code, ".reduce(function (previous, next) { "
                ...
                [&](std::unique_ptr<types::structure> const &root)
                {
                    ...
                    for (types::structure::element const &e : root->elements)
                    {
                        ...
                        generate_size_of_value(code, indentation, e.what,
                                               Si::make_memory_range(element_value), library);
                    }
                    Si::append(code, ")");
                });
        ...
        template <class CharSink>
        void generate_serialization(CharSink &&code, indentation_level indentation,
                                    types::type const &type, Si::memory_range const value,
                                    Si::memory_range const destination,
                                    Si::memory_range const offset, Si::memory_range const library)
        {
            Si::visit<void>(
                ...
                [&](types::integer const range)
                {
                    ...
                    {
                    case 1:
                        start_line(code, indentation, "new DataView(", destination, ").setUint8(",
                                   offset, ", ", value, ");\n");
                        break;

                    case 2:
                        start_line(code, indentation, "new DataView(", destination, ").setUint16(",
                                   offset, ", ", value, ");\n");
                        break;

                    case 4:
                        start_line(code, indentation, "new DataView(", destination, ").setUint32(",
                                   offset, ", ", value, ");\n");
                        break;
                          ...
                          [&](indentation_level const in_function)
                          {
                              ...
                              for (size_t i = 0, c = root->elements.size(); i < c; ++i)
                              {
                                  ...
                                  generate_serialization(code, indentation, root->elements[i],
                                                         Si::make_memory_range(element_value),
                                                         destination,
                ...
                [&](types::utf8 const text)
                {
                    ...
                    generate_serialization(code, indentation, text.code_units,
                                           Si::make_memory_range(length), destination, offset,
                                           library);
                    ...
                    content_offset.append(")");
                    start_line(code, indentation, "new Uint8Array(", destination,
                               ").set(new Uint8Array(", library, ".to_utf8(", value, ")), ",
                               content_offset, ");\n");
                },
                [&](std::unique_ptr<types::structure> const &root)
                {
                                      ...
                                      Si::memory_range const common_library)
        {
            ...
            block(
                ...
                [&](indentation_level const in_state)
                {
                    ...
                    generate_parser_creation(code, in_state, types::utf8{types::integer{0, 255}},
                                             common_library);
                    Si::append(code, ";\n");
                    start_line(code, in_state, "state = function (input)\n");
                    block(
                        ...
                        [&](indentation_level const in_if)
                        {
                                    ...
                                    {
                                        {
                                            ...
                                            for (types::parameter const &parameter :
                                                 method.second.parameters)
                                            {
                                                parameters.elements.emplace_back(
                                                    clone(parameter.type_));
                                            }
                                            start_line(code, in_if_2, "var parameters = ");
                                            types::type const parameters_erased =
                                                Si::to_unique(std::move(parameters));
                                            generate_parser_creation(
                                                code, in_if_2, parameters_erased, common_library);
                                            Si::append(code, ";\n");
                                        }
                                        ...
                                        block(
                                            ...
                                            [&](indentation_level const in_function)
                                            {
                                                start_line(code, in_function, "var status = "
                                                                              "parameters(input);"
                                                                              "\n");
                                                ...
                                                start_line(code, in_function,
                                                           "state = initial_state;\n");
                                                start_line(code, in_function,
                                                           "server_implementation.", method.first,
                                                           "(");
                                                auto comma = warpcoil::make_comma_separator(
                                                    Si::ref_sink(code));
                                                for (size_t i = 0;
                                                     i < method.second.parameters.size(); ++i)
                                                {
                                                    comma.add_element();
                                                ...
                                                block(
                                                    ...
                                                    {
                                                        start_line(code, in_callback,
                                                                   "var response_size "
                                                                   "= 1 + 8 + ");
                                                        generate_size_of_value(
                                                            code, in_callback, method.second.result,
                            ...
                            start_line(code, in_if, "throw new Error(\"unknown "
                                                    "method: \" + status);\n");
                        },
                        ";\n");
                },
            ...
            block(code, indentation,
                  [&](indentation_level const in_function)
                  {
                      ...
                      block(code, in_function,
                            [&](indentation_level const in_first_state)
                            {
                                ...
                                block(code, in_first_state,
                                      [&](indentation_level const in_if)
                                      {
                                          generate_request_parsing(code, in_if, interface,
                                                                   common_library);
                                      },
                                      "\n");
                                ...
                                block(code, in_first_state,
                                      [&](indentation_level const in_else)
                                      {
                                          start_line(code, in_else, "throw new Error(\"unknown "
                                                                    "message type\");\n");
                                      },
                                      "\n");
                            },
                      ...
                      block(code, in_function,
                            ...
                            {
                                start_line(code, in_parse, common_library,
                                           ".assert(undefined === state(input));\n");
                            },
                            ";\n");
                  },
                  "");
                        ...
                        [&](indentation_level const in_result)
                        {
                                 ...
                                 ++i)
                            {
                                ...
                                block(
                                    ...
                                    {
                                        start_line(
                                            code, in_method, "var request_size = 1 + 8 + 1 + ",
                                             ...
                                             method.second.parameters)
                                        {
                                            Si::append(code, " + ");
                                            generate_size_of_value(
                                                code, in_method, parameter.type_,
                                                Si::make_memory_range(parameter.name), library);
                                        }
                                        Si::append(code, ";\n");
                                        start_line(code, in_method, "var request_buffer = new "
                                                                    "ArrayBuffer(request_size);"
                                                                    "\n");
                                        generate_serialization(
                                            code, in_method, types::integer{0, 255},
                                            Si::make_c_str_range("0"),
                                            Si::make_c_str_range("request_buffer"),
                                            Si::make_c_str_range("0"), library);
                                        start_line(code, in_method, "var request_id = {high: 0, "
                                                                    "low: next_request_id};\n");
                                        // TODO: support more than 32 bit
                                        // request IDs
                                        start_line(code, in_method, "++next_request_id;\n");
                                        generate_serialization(
                                            code, in_method, types::integer{0, 0xffffffffffffffffu},
                                            Si::make_c_str_range("request_id"),
                                            Si::make_c_str_range("request_buffer"),
                                        ...
                                        start_line(code, in_method, "var write_pointer = 1 + 8;\n");
                                        {
                                            ...
                                            method_name_literal += "\"";
                                            types::type const name_type =
                                                types::utf8{types::integer{0, 255}};
                                            generate_serialization(
                                                code, in_method, name_type,
                                        ...
                                        {
                                            types::parameter const &parameter =
                                                method.second.parameters[k];
                                            generate_serialization(
                                                code, in_method, parameter.type_,
                                        ...
                                        start_line(code, in_method,
                                                   "send_bytes(request_buffer);\n");
                                        start_line(code, in_method, "var key = "
                                                                    "JSON.stringify(request_id);"
                                                                    "\n");
                                        start_line(code, in_method, "pending_requests[key] = "
                                                                    "{argument_parser: ");
                                        generate_parser_creation(code, in_method,
                                                                 method.second.result, library);
            ...
            Si::append(code, "{\n");
            {
                ...
                start_line(code, in_object, "to_utf8: function (string)\n");
                {
                    block(code, in_object,
                          [&](indentation_level const in_function)
                          {
                              start_line(code, in_function, "var buffer = new "
                                                            "ArrayBuffer(string.length);\n");
                              start_line(code, in_function, "var view = new DataView(buffer);\n");
                              start_line(code, in_function, "for (var i = 0, c "
                                                            "= string.length; "
                                                            "i < c; ++i)\n");
                              block(code, in_function,
                                    [&](indentation_level const in_for)
                                    {
                                        ...
                                        start_line(code, in_for, "view.setUint8(i, "
                                                                 "string.charCodeAt(i));\n");
                                    },
                                    "\n");
                ...
                Si::append(code, "function (x) { if (!(x)) { throw new "
                                 "Error(\"assertion failed\"); } },\n");

                start_line(code, in_object, "assert_eq: ");
                Si::append(code, "function (expected, got) { if (expected !== "
                                 "got) { throw new Error(\"Expected \" + "
                                 "expected + \", got \" + got); } },\n");

                start_line(code, in_object, "require_type: ");
                Si::append(code, "function (type, value) { "
