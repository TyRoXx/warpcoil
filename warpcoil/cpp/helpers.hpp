...
namespace warpcoil
{
	namespace cpp
	{
		...
		void begin_parse_response(AsyncReadStream &responses, Buffer response_buffer, std::size_t &response_buffer_used,
		                          Parser parser, ResultHandler &&on_result)
		{
			for (std::size_t i = 0; i < response_buffer_used; ++i)
			{
				std::uint8_t *const data = boost::asio::buffer_cast<std::uint8_t *>(response_buffer);
				if (auto *response = parser.parse_byte(data[i]))
				{
					std::copy(data + i + 1, data + response_buffer_used, data);
					response_buffer_used -= 1 + i;
			...
			responses.async_read_some(
			    response_buffer,
			    [&responses, response_buffer, &response_buffer_used, parser = std::move(parser), on_result ](
			        boost::system::error_code ec, std::size_t read) mutable
			    {
				    ...
				    response_buffer_used = read;
				    begin_parse_response(responses, response_buffer, response_buffer_used, std::move(parser),
				                         on_result);
				});
		}
	}
