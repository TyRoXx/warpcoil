...
namespace warpcoil
{
    namespace cpp
    {
        ...
        void generate_value_serialization(CharSink1 &&code,
                                          shared_code_generator<CharSink2> &shared,
                                          indentation_level indentation, Si::memory_range sink,
                                          ...
                                          Si::memory_range return_invalid_input_error)
        {
            return Si::visit<void>(
                ...
                [&](types::integer range)
                {
                    ...
                    {
                        start_line(code, indentation, "if (((", value, ") < ",
                                   boost::lexical_cast<std::string>(range.minimum), "u) || ((",
                                   value, ") > ", boost::lexical_cast<std::string>(range.maximum),
                                   "u)) { ", return_invalid_input_error, "; }\n");
                    }
                    start_line(code, indentation, "warpcoil::cpp::write_integer(", sink,
                               ", static_cast<", find_suitable_uint_cpp_type(range), ">(", value,
                               "));\n");
                },
                [&](std::unique_ptr<types::variant> const &root)
                {
                    ...
                    generate_value_serialization(
                        code, shared, indentation, sink, Si::make_contiguous_range(index),
                        types::integer{0, root->elements.size() - 1}, return_invalid_input_error);
                    ...
                    block(code, indentation,
                          [&](indentation_level const in_switch)
                          {
                              ...
                              for (types::type const &element : root->elements)
                              {
                                  ...
                                  generate_type(Si::make_container_sink(element_expression), shared,
                                                element);
                                  element_expression += ">(";
                                  element_expression.insert(element_expression.end(), value.begin(),
                                                            value.end());
                                  element_expression += "))";
                                  generate_value_serialization(
                                      code, shared, in_case, sink,
                ...
                {
                    {
                        ...
                        size += ".size()";
                        generate_value_serialization(code, shared, indentation, sink,
                                                     Si::make_contiguous_range(size), root->length,
                                                     return_invalid_input_error);
                    }
                    ...
                    Si::append(code, "{\n");
                    {
                        ...
                        generate_value_serialization(code, shared, in_for, sink,
                                                     Si::make_c_str_range("element"), root->element,
                                                     return_invalid_input_error);
                    }
                    indentation.render(code);
                    Si::append(code, "}\n");
                ...
                {
                    start_line(code, indentation, "if (!utf8::is_valid(", value, ".begin(), ",
                               value, ".end())) { ", return_invalid_input_error, "; }\n");
                    {
                        std::string size;
                        size.append(value.begin(), value.end());
                        size += ".size()";
                        generate_value_serialization(code, shared, indentation, sink,
                                                     Si::make_contiguous_range(size),
                                                     text.code_units, return_invalid_input_error);
                    }
                    start_line(code, indentation, "Si::append(");
                    Si::append(code, sink);
                    Si::append(code, ", "
                                     "Si::make_iterator_range(reinterpret_cast<"
                                     "std::uint8_t const *>(");
                    Si::append(code, value);
